...
export class CreateOwnNinjaComponent {
  ...
  validateBody(body:ICreateUserNinja){
    ...
    if(body.accesories //&& body.accesories.accesorieSetName.length >0 
      && body.accesories.accesorieSetName.length <5 && body.accesories.accesories.length > 0){
      this.showError("Ninja accesories set name must have 5 or more characters");
      return false;
    }
    if(body.set //&& body.set.setName.length >0
      && body.set.setName.length < 5  && body.set.equipment.length > 0){
      this.showError("Ninja equipment set name must have 5 or more characters");
      return false;
    }

    if(body.accesories //&& body.accesories.accesorieSetName.length >0 
      && body.accesories.accesorieSetName.length >=0 && body.accesories.accesories.length == 0){
      //this.showError("Ninja accesories set name must have 5 or more characters");
      this.hijoComponent.accesorieSetName = "";
      body.accesories = undefined;
    }

    if(body.set //&& body.set.setName.length >0 .length < 5
      && body.set.setName.length >= 0  && body.set.equipment.length == 0){
      this.hijoComponent.setName = "";
      body.set = undefined;
    }
    return true;
  }
  ...
  saveNinja(){
    if(this.ninjaShow.ninja && this.ninjaName && this.ninjaName.length > 4 ){
      ...
      //console.log(this.validateBody(body));
      if(this.validateBody(body)){
