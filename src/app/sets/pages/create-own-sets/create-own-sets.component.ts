...
export class CreateOwnSetsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.setItemsService.getSetItems().pipe(
      concatMap((response) => {
        this.items = response;
        this.rechargeItemsList = false;
        setTimeout(() => {
      ...
      if (response) {
        ...
        } );///this.addSetItem(item)
        this.setName = response.nombre;
        console.log("si hay set");
      } else {
        console.log("No hay set a modificar");
      }
    });
    /*console.log("Me rehago");
    this.setdataSharedService.getUserSetToModify.subscribe(
      response =>{
        if(response){ 
          console.log("ulala");
         console.log(this.items);
          
          response.partes.forEach(
            item => this.addSetItem(item)
          );
          this.setName = response.nombre;
          console.log("si hay set")
        }else{
    ...
    /*this.setItemsService.getSetItems().subscribe(
      response =>{
        this.items = response;
        this.rechargeItemsList = false;
        setTimeout(() => {
